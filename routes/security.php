...
Route::middleware('role:security')->group(function (): void {
    // Checkpoint Selection (Exempt from EnsureCheckpointSelected)
    Route::prefix('checkpoint')->name('security.checkpoint.')->group(function (): void {
        Route::get('/select', [CheckpointController::class, 'select'])->name('select');
        Route::post('/claim', [CheckpointController::class, 'claim'])->name('claim');
        Route::post('/release', [CheckpointController::class, 'release'])->name('release');
    });

    // Operational Workspace Routes (Protected by EnsureCheckpointSelected)
    Route::middleware(EnsureCheckpointSelected::class)->group(function (): void {
        // Home - Command Center
