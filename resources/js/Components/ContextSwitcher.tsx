    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={clsx(
                    'group flex items-center gap-2 rounded-xl px-2.5 py-1.5 text-left transition-all',
                    variant === 'dark'
                        ? 'hover:bg-white/10 active:bg-white/15'
                        : 'border border-slate-200/80 bg-white/70 shadow-xs hover:bg-slate-100/80 active:bg-slate-200/70',
                )}
            >
                <div className="flex min-w-0 flex-col justify-center text-left">
                    ...
                    <div className="mt-0.5 flex items-center gap-1.5">
                        <div className="shrink-0">{getRoleIcon(currentContext.role_name, 'w-3 h-3')}</div>
                        <p
                            className={clsx(
                                'truncate text-[11px] leading-tight font-medium capitalize',
            ...
            {isOpen && (
                <div
                    className={clsx(
                        'animate-in fade-in zoom-in-95 absolute z-50 mt-2 w-72 max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border shadow-xl backdrop-blur-xl duration-150',
                                    ...
                                    className={clsx(
                                        ...
                                        isSelected
                                            ? variant === 'dark'
                                                ...
                                                : 'bg-emerald-50 font-semibold text-emerald-800 ring-1 ring-emerald-600/20'
                                            : variant === 'dark'
                                ...
                                >
                                    <div className="flex min-w-0 items-center gap-3 pr-2">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="truncate text-sm leading-tight font-semibold">{ctx.estate_name}</p>
                                            <p
                                                className={clsx(
                                                    'mt-0.5 truncate text-xs leading-tight capitalize',
                                                    isSelected
                                                        ? variant === 'dark'
                                                            ? 'text-emerald-400/80'
                                                            : 'text-emerald-600'
                                    ...
                                    {isSelected && (
                                        <Check className={clsx('h-4 w-4 shrink-0', variant === 'dark' ? 'text-emerald-400' : 'text-emerald-600')} />
                                    )}
                                </button>
                            );
                        })}
