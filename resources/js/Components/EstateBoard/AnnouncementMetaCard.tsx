        ...
        <div className={`rounded-3xl border border-slate-200/80 bg-white p-5 shadow-xs sm:p-6 ${className}`}>
            ...
            <div className="space-y-3.5 text-left text-xs">
                ...
                <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                    ...
                    <span className="font-bold text-slate-800">{publishedAt ? formatDate(publishedAt) : formatDate(createdAt)}</span>
                </div>
                ...
                <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                    ...
                    <span
                        className={`text-[10px] font-black uppercase ${
                            priority === 'critical' ? 'text-rose-600' : priority === 'important' ? 'text-amber-600' : 'text-slate-600'
                        }`}
                    >
                        {priority}
                    </span>
                ...
                <div className="flex items-center justify-between pt-0.5">
                    ...
                    <span className={`font-bold capitalize ${status === 'published' ? 'text-emerald-600' : 'text-slate-600'}`}>{status}</span>
                </div>
            </div>
        </div>
    );
