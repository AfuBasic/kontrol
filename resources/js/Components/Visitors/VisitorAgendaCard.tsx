...
export default function VisitorAgendaCard({
    ...
    const statusConfig = statusMap[code.status] ??
        DEFAULT_STATUS_MAP[code.status] ?? {
            label: code.status,
    ...
    const timeLabel = isHistoryItem ? (code.completion_time ?? null) : (code.arrival_time ?? null);

    const typeLabel = code.type === 'single_use' ? 'One-Time' : code.type === 'long_lived' ? 'Long-Term' : 'Event';

    const rowContent = (
        <div className="group relative flex items-center gap-3 rounded-lg px-1 py-2 transition-colors hover:bg-slate-50/90 active:bg-slate-100/80">
            {/* 1. Time Column (Left-aligned, prominent) */}
            {showTime && (
                <div className="w-16 shrink-0 text-right">
                    <span
                        className={`text-xs font-semibold tracking-tight tabular-nums ${
                            isHistoryItem ? 'font-normal text-slate-400' : 'text-slate-700'
                        }`}
                    >
                        {timeLabel || 'Anytime'}
                    </span>
            ...
            <div className="relative flex shrink-0 flex-col items-center">
                ...
                {!isLastInGroup && <div className="absolute top-4 bottom-0 w-px bg-slate-200/80" />}
            </div>
            ...
            <div className="flex min-w-0 flex-1 items-baseline justify-between gap-2">
                <div className="flex min-w-0 flex-wrap items-baseline gap-2">
                    <span
                        className={`truncate text-sm font-semibold tracking-tight ${isHistoryItem ? 'font-normal text-slate-500' : 'text-slate-900'}`}
                    >
                        {visitorName}
                    </span>

                    {code.purpose && <span className="truncate text-xs font-normal text-slate-400">· {code.purpose}</span>}

                    {code.notes && <StickyNote className="h-3 w-3 shrink-0 text-slate-300" />}
                </div>

                {/* Status & Actions on right edge */}
                <div className="flex shrink-0 items-center gap-2">
                    {renderMeta ? renderMeta(code) : <span className="hidden text-[11px] font-normal text-slate-400 sm:inline">{typeLabel}</span>}

                    {showStatus && <span className={`text-[11px] ${statusConfig.textClass}`}>{statusConfig.label}</span>}

                    {renderActions && <div className="opacity-70 transition-opacity group-hover:opacity-100">{renderActions(code)}</div>}
                </div>
            </div>
        </div>
    );
