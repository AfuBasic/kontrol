    ...
    return (
        <section
            className={
                isSecurity
                    ? className
                    : `rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs sm:p-5 ${dark('dark:border-slate-800 dark:bg-slate-900')} ${className}`
            }
        >
            <div
                className={
                    isSecurity
                        ...
                        : `mb-4 flex items-center justify-between border-b border-slate-100 pb-3 sm:pb-4 ${dark('dark:border-slate-800')}`
                }
            >
                <div className="flex min-w-0 items-center gap-2">
                    {!isSecurity && <MessageSquare className="h-4 w-4 shrink-0 text-slate-500" />}
                    <h3
                        className={
                            isSecurity
                    ...
                    <span
                        className={
                            isSecurity
                                ...
                                : `shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-600 ${dark('dark:bg-slate-800 dark:text-slate-400')}`
                        }
                    >
                        {commentList.length}
                    </span>
            ...
            {isSecurity && <p className="mb-3 text-xs font-medium text-slate-500">Share updates or discuss this incident.</p>}

            {canComment && onSubmitComment && (
                <form onSubmit={handleSubmit} className={isSecurity ? 'mb-4' : 'mb-5'} noValidate>
                    {replyTo && (
                        <div
                    ...
                    {isSecurity ? (
                        <div className="flex flex-col gap-2 min-[360px]:flex-row min-[360px]:items-end">
                            <textarea
                                value={body}
                                ...
                                rows={2}
                                disabled={submitting}
                                className="w-full min-w-0 resize-none rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-900 placeholder:text-slate-400 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 focus:outline-hidden disabled:bg-slate-50 disabled:text-slate-500 min-[360px]:flex-1"
                            />
                            <button
                                type="submit"
                                disabled={!body.trim() || submitting}
                                className="inline-flex h-9 w-full shrink-0 items-center justify-center gap-1.5 rounded-xl bg-indigo-600 px-3 text-xs font-bold text-white shadow-xs transition-all hover:bg-indigo-700 active:scale-95 disabled:cursor-not-allowed disabled:bg-indigo-600/50 disabled:text-white min-[360px]:w-auto"
                            >
            ...
            {commentList.length === 0 ? (
                isSecurity ? (
                    <p className="border-t border-slate-100 pt-3 text-xs text-slate-500">No comments yet. Be the first to share details.</p>
                ) : (
                    <div
                ...
                <div className={isSecurity ? 'divide-y divide-slate-100 border-t border-slate-100' : 'space-y-3.5'}>
                    {commentList.map((comment) => (
                        <div key={comment.id} className={isSecurity ? 'py-3.5' : 'space-y-2.5'}>
                            {isSecurity ? (
                                <article>
                                    <div className="flex items-start gap-2.5">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex flex-col gap-0.5 min-[380px]:flex-row min-[380px]:items-baseline min-[380px]:justify-between min-[380px]:gap-3">
                                                ...
                                                <time className="shrink-0 text-[11px] font-medium text-slate-500" dateTime={comment.created_at}>
                                                    {formatDate(comment.created_at)}
                                                </time>
                                            </div>
                                            <p className="mt-1 text-xs leading-relaxed whitespace-pre-line text-slate-700 sm:text-sm">
                                                {comment.body}
                                            </p>
                                            {canComment && onSubmitComment && (
                                                <button
                                                    ...
                                                    className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold text-slate-500 transition-colors hover:text-indigo-600"
                                                >
                                            ...
                                            {comment.replies.map((reply) => (
                                                <div key={reply.id} className="min-w-0">
                                                    <div className="flex flex-col gap-0.5 min-[380px]:flex-row min-[380px]:items-baseline min-[380px]:justify-between min-[380px]:gap-3">
                                                        ...
                                                        <time className="shrink-0 text-[11px] font-medium text-slate-500" dateTime={reply.created_at}>
                                                            {formatDate(reply.created_at)}
                                                        </time>
                                                    </div>
                                                    <p className="mt-1 text-xs leading-relaxed whitespace-pre-line text-slate-700">{reply.body}</p>
                                                </div>
                                            ))}
                                        </div>
                                    )}
