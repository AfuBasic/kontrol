...
export default function Login() {
    ...
    async function handleGoogleSignIn() {
        ...
        try {
            // Use Firebase Authentication for native OAuth
            const result = await FirebaseAuthentication.signInWithGoogle();

            // Extract the ID token from the credential object (Capawesome v8 structure)
            const idToken = result.credential?.idToken;
            if (idToken) {
                // Send the ID token to backend for verification and login
                router.post(SocialLoginController.handleGoogleMobileToken.url(), {
                    token: idToken,
                }, {
            ...
            // Show error to user
            console.error('Google Native Sign-In failed:', JSON.stringify(err, Object.getOwnPropertyNames(err)));
    ...
    return (
        <>
                    ...
                    <div className="relative z-10 mx-auto w-full max-w-sm">
                        ...
                        {(flash?.error || errors?.email) && (
                            <motion.div
                                initial={{ opacity: 0, y: -10 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="mb-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 font-medium"
                            >
                                {flash?.error || errors?.email}
                            </motion.div>
                        )}
                        ...
                        >
                            ...
                            <button
                                ...
                                className="flex w-full items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3.5 text-sm font-medium text-gray-700 transition-all hover:border-gray-300 hover:bg-gray-50 focus:ring-2 focus:ring-[#1F6FDB]/20 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                            >
                                {googleLoading ? (
            ...
            {/* Error Toast for Google Sign-In */}
            <Toast
