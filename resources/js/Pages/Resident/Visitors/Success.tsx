...
export default function CodeSuccess({ accessCode }: Props) {
    ...
    async function handleShare() {
        ...
        setSharing(true);
        try {
            await shareAccessCode(accessCode, cardRef.current);
        } finally {
            setSharing(false);
        }
    }
            ...
            <div className="flex min-h-[60vh] flex-col items-center justify-center py-4 text-center">
                ...
                >
                    ...
                    <button
                        onClick={handleShare}
                        disabled={sharing}
                        className="flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-gray-200 bg-white px-5 py-3.5 text-sm font-semibold text-gray-700 transition-all hover:bg-gray-50 active:scale-[0.98] disabled:opacity-75 disabled:pointer-events-none"
                    >
                        {sharing ? (
                            <>
