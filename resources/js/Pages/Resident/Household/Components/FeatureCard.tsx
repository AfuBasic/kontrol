...
export default function FeatureCard({ title, description, icon, color, delay = 0 }: Props) {
    return (
        ...
        >
            <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${color}`}>{icon}</div>
            <div>
                <h4 className="text-sm font-black text-slate-900">{title}</h4>
                <p className="mt-1 text-xs leading-relaxed font-medium text-slate-400">{description}</p>
            </div>
        </motion.div>
    );
}
