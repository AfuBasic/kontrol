    ...
    return (
        <OrganizationLayout title="Home" contentClassName="max-w-[92rem]">
            ...
            <div className="space-y-5">
                {isQuiet ? (
                    <section className="relative overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-[#0c192c] via-[#0f233d] to-[#0a1322] p-5 text-white shadow-[0_24px_70px_rgba(10,25,47,0.22)] ring-1 ring-white/10 sm:rounded-[2rem] sm:p-8 lg:p-10">
                        ...
                        <div className="relative max-w-2xl space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <h1 className="text-2xl font-bold tracking-tight text-white sm:text-4xl">
                                    {organization.name} is quiet today
                                </h1>
                                <p className="max-w-lg text-sm leading-relaxed font-normal text-slate-300 sm:text-base">
                                    No one is currently checked in. When guests or visitors arrive at the gate, live notifications and confirmation requests will appear here.
                                </p>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-3 pt-2">
                                <Link
                                    ...
                                    className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-2.5 text-sm font-bold text-slate-900 shadow-sm transition hover:bg-slate-100"
                                >
                                ...
                                <Link
                                    ...
                                    className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white/10 px-5 py-2.5 text-sm font-semibold text-white ring-1 ring-white/15 transition hover:bg-white/15"
                                >
                            ...
                            <div className="relative flex h-full flex-col justify-between gap-6 sm:gap-10">
                                <div className="space-y-4 sm:space-y-5">
                                    ...
                                    <div className="max-w-2xl space-y-2 sm:space-y-3">
                                        <p className="text-xs font-medium tracking-normal text-slate-300 sm:text-sm">
                                            {getGreeting()}, {userFirstName}
                                        </p>
                                        <h1 className="text-2xl font-bold tracking-tight text-white sm:text-5xl sm:leading-[1.1]">
                                            {organization.name} today
                                        </h1>
                                        <p className={`max-w-lg text-xs leading-relaxed font-normal sm:text-base ${operatingState.tone}`}>
                                ...
                                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                                    <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.06] p-2.5 backdrop-blur-xs transition-colors sm:rounded-[1.35rem] sm:p-4">
                                        <p className="text-xl font-bold tracking-tight text-white sm:text-3xl sm:font-extrabold">{currentlyHere}</p>
                                        <p className="mt-1 text-[11px] font-medium tracking-normal text-slate-300 sm:text-xs">Here now</p>
                                    </div>
                                    <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.06] p-2.5 backdrop-blur-xs transition-colors sm:rounded-[1.35rem] sm:p-4">
                                        <p className="text-xl font-bold tracking-tight text-white sm:text-3xl sm:font-extrabold">{arrivalsToday}</p>
                                        <p className="mt-1 text-[11px] font-medium tracking-normal text-slate-300 sm:text-xs">Arrivals today</p>
                                    </div>
                                    <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.06] p-2.5 backdrop-blur-xs transition-colors sm:rounded-[1.35rem] sm:p-4">
                                        <p className="text-xl font-bold tracking-tight text-white sm:text-3xl sm:font-extrabold">{confirmedToday}</p>
                                        <p className="mt-1 text-[11px] font-medium tracking-normal text-slate-300 sm:text-xs">Confirmed</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <section className="grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
                    ...
                    <div className="rounded-[1.5rem] bg-white p-4 shadow-[0_18px_55px_rgba(15,23,42,0.07)] ring-1 ring-slate-200/80 sm:rounded-[2rem] sm:p-6">
                        ...
                        {recent_activity.length === 0 ? (
                            <div className="mt-6 flex gap-4 rounded-[1.5rem] bg-slate-50 p-5">
                            ...
                            <div className="mt-5 space-y-1">
                                {recent_activity.slice(0, 8).map((item) => (
                                    <div key={item.id} className="flex flex-col gap-1 py-3 sm:grid sm:grid-cols-[4.6rem_minmax(0,1fr)] sm:items-start sm:gap-3">
                                        <time className="hidden text-right text-xs font-medium text-slate-400 sm:block">{item.time_human}</time>
                                        <div className="flex items-start gap-3">
                                            <span
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-sm leading-relaxed font-normal text-slate-700">{item.description}</p>
                                                <time className="mt-0.5 block text-xs font-medium text-slate-400 sm:hidden">{item.time_human}</time>
                                            </div>
                                        </div>
                                    </div>
                                ))}
