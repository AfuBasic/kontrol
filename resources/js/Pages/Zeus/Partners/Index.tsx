            ...
            >
                <div>
                    ...
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900">
                        Strategic <span className="font-light text-slate-400">Partners</span>
                    </h1>
                </div>
                <Link
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="border-b border-slate-100 bg-white">
                            <tr>
                                <th className="px-6 py-4 text-left text-[10px] font-bold tracking-[0.15em] text-slate-400 uppercase">Partner Details</th>
                                <th className="px-6 py-4 text-left text-[10px] font-bold tracking-[0.15em] text-slate-400 uppercase">Commission</th>
                                <th className="px-6 py-4 text-center text-[10px] font-bold tracking-[0.15em] text-slate-400 uppercase">Network Size</th>
                                <th className="px-6 py-4 text-left text-[10px] font-bold tracking-[0.15em] text-slate-400 uppercase">Current State</th>
                            ...
                            {partners.data.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                                        {hasFilters ? 'No partners match your filters.' : 'No partners found. Create one to get started!'}
                                    </td>
                                </tr>
                            ) : (
                                partners.data.map((partner) => (
                                    <tr key={partner.id} className="bg-white transition-colors hover:bg-slate-50/50">
                                        <td className="px-6 py-5">
                                            <div>
                                                <p className="text-[13px] font-bold text-slate-900">{partner.name}</p>
                                                <p className="text-[11px] text-slate-500">{partner.email}</p>
                                                {partner.contact_person && (
                                                    <p className="mt-0.5 text-[10px] text-slate-400">Attn: {partner.contact_person}</p>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-[13px] font-black text-slate-900">{partner.commission_rate}%</span>
                                            <span className="ml-1 text-[10px] font-bold text-slate-300">COMM</span>
                                        </td>
                                        <td className="px-6 py-5 text-center">
                                            <span className="text-[14px] font-bold text-slate-700">{partner.estates_count}</span>
                                        </td>
                                        <td className="px-6 py-5 text-left">
                                            <span
                                                className={`inline-flex items-center gap-1.5 rounded px-2 py-0.5 text-[10px] font-bold tracking-tight uppercase ${
                                                    partner.status === 'active' ? 'bg-blue-500/10 text-blue-500' : 'bg-slate-100 text-slate-500'
                                                }`}
                                            >
                                                {partner.status}
                                            </span>
