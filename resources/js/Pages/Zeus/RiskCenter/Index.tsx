            ...
            <motion.div variants={containerVariants} initial="hidden" animate="show" className="mx-auto max-w-7xl">
                ...
                <motion.div variants={itemVariants} className="mb-10 grid gap-4 sm:grid-cols-3">
                    <div className="rounded-3xl border border-rose-200 bg-white p-6 shadow-sm dark:border-rose-500/30 dark:bg-[#0f1423]">
                        ...
                        <p className="mt-2 text-3xl font-bold text-slate-900 dark:text-white truncate" title={formatCurrency(totalMrrAtRisk)}>
                ...
                <div className="grid gap-8 lg:grid-cols-3">
                    {/* The Risk List */}
                    <motion.div variants={itemVariants} className="lg:col-span-2 overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm dark:border-slate-800/50 dark:bg-[#0f1423]">
                        <div className="border-b border-slate-50 px-8 py-6 dark:border-slate-800/30 flex items-center justify-between">
                            <div>
                        ...
                        {riskList.length > 0 ? (
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    <thead className="bg-slate-50/50 dark:bg-slate-800/20">
                                        <tr>
                                            <th className="px-6 py-3 text-[10px] font-bold tracking-widest text-slate-400 uppercase dark:text-slate-500">Account</th>
                                            <th className="px-6 py-3 text-[10px] font-bold tracking-widest text-slate-400 uppercase dark:text-slate-500">Risk Factors</th>
                                            <th className="px-6 py-3 text-[10px] font-bold tracking-widest text-slate-400 uppercase dark:text-slate-500">MRR Impact</th>
                                            <th className="px-6 py-3 text-[10px] font-bold tracking-widest text-slate-400 uppercase dark:text-slate-500">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800/30">
                                        {riskList.map((item) => (
                                            <tr key={item.id} className={`transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/20 ${item.risk_level === 'critical' ? 'bg-rose-50/30 dark:bg-rose-500/5' : ''}`}>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-[10px] flex items-center gap-2 mt-1">
                                                                <span className={`inline-flex rounded px-1.5 py-0.5 text-[9px] font-bold uppercase ${
                                                                    item.risk_level === 'critical' ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400'
                                                                }`}>
                                                                    {item.risk_level}
                                                                </span>
                                                                {item.health_score !== null && (
                                                                    <span className="text-slate-500 dark:text-slate-400">Score: {item.health_score}</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-col gap-1">
                                                        {item.risk_factors.map((factor, idx) => (
                                                            <span key={idx} className="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-300">
                                                ...
                                                <td className="px-6 py-4">
                                                    {item.entity_type === 'estate' && (
                                                        <Link href={`/zeus/estates/${item.entity_id}`} className="inline-flex items-center justify-center rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 hover:bg-slate-50 dark:bg-slate-800 dark:text-white dark:ring-slate-700 dark:hover:bg-slate-700">
                                                            Investigate
                                                        </Link>
                                                    )}
                                                </td>
                            ...
                            <div className="flex flex-col items-center justify-center p-16 text-center">
                                <ShieldAlert className="h-12 w-12 text-emerald-400 dark:text-emerald-500/50 mb-4" />
                    ...
                    <motion.div variants={itemVariants} className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm dark:border-slate-800/50 dark:bg-[#0f1423] flex flex-col h-[600px]">
                        <div className="border-b border-slate-50 px-8 py-6 dark:border-slate-800/30 shrink-0">
                            <h3 className="flex items-center gap-2 text-sm font-bold tracking-wider text-slate-900 uppercase dark:text-white">
                                <Activity className="h-4 w-4 text-sky-500" />
                                Platform Heartbeat
                            </h3>
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                Real-time feed of major business events.
                            </p>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 pt-4">
                            {activityStream.length > 0 ? (
                                <div className="relative space-y-6 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-800 before:to-transparent">
                                    {activityStream.map((activity, idx) => (
                                        <div key={activity.id} className="relative flex items-start gap-6">
                                            <div className="flex shrink-0 items-center justify-center relative z-10 mt-1">
                                                <div className={`flex h-10 w-10 items-center justify-center rounded-full ring-4 ring-white dark:ring-[#0f1423] shadow-sm ${
                                                    activity.type === 'success' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400' :
                                                    activity.type === 'danger' ? 'bg-rose-100 text-rose-600 dark:bg-rose-500/20 dark:text-rose-400' :
                                                    activity.type === 'warning' ? 'bg-amber-100 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400' :
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center">
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Platform events will stream here.</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
