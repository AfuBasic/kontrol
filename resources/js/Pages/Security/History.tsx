        ...
        <>
            ...
            <div className="pb-32 pt-2">
                <div className="mb-8 px-2">
                    <h1 className="text-3xl font-black tracking-tight text-slate-900 mb-1">Access History</h1>
                        ...
                        >
                            {logs.data.map((log) => (
                                <div 
                                    ...
                                    className="group relative overflow-hidden rounded-[2.25rem] bg-white p-6 ring-1 ring-slate-200 shadow-xs transition-all active:scale-[0.98] active:bg-slate-50"
                                >
                                    <div className="flex items-start justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-50 text-indigo-600 ring-1 ring-slate-100 shadow-inner">
                                                <User className="h-7 w-7" />
                                            </div>
                                            <div>
                                                <h3 className="text-lg font-bold text-slate-900 leading-tight">{log.visitor.name}</h3>
                                                <p className="text-xs font-semibold text-slate-500 mt-0.5">Host: {log.host.name}</p>
                                            </div>
                                        </div>
                                        <div className="flex flex-col items-end gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{log.verified_at_human}</span>
                                        </div>
                                    </div>
                                    ...
                                    {log.vehicle && (
                                        <div className="mt-5 flex items-center justify-between rounded-[1.5rem] bg-slate-50 p-4 ring-1 ring-slate-100">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">Vehicle</p>
                                                    <p className="text-sm font-bold text-slate-900">{log.vehicle.make} {log.vehicle.model}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">Plate</p>
                                    ...
                                    <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Guard</span>
                ...
                <div className="space-y-6 pt-4">
                    ...
                    <div className="flex gap-4 pt-4">
                        <button 
                            ...
                            className="flex-1 rounded-[1.25rem] bg-slate-100 py-4 text-sm font-black text-slate-600 active:scale-95 transition-transform"
                        >
                            Reset
                        </button>
                        <button 
                            onClick={() => setIsFilterVisible(false)}
                            className="flex-[2] rounded-[1.25rem] bg-indigo-600 py-4 text-sm font-black text-white shadow-xl shadow-indigo-500/20 active:scale-95 transition-transform"
                        >
                            Apply Filters
                        </button>
                    </div>
                    ...
                    <div className="space-y-8 pt-4">
                        ...
                        <div className="flex flex-col items-center text-center">
                            ...
                            <div className="mt-2 flex items-center gap-2">
                                ...
                                <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{selectedLog.visitor.type || 'Standard Visitor'}</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-5">
                            ...
                            <div className="rounded-[2rem] bg-slate-50 p-6 ring-1 ring-slate-200/50">
                                <div className="mb-5 flex items-center gap-3">
                                    ...
                                    <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">Host & Location</h4>
                                </div>
                                <div className="space-y-4">
                                    <div>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Resident Name</p>
                                        <p className="text-lg font-bold text-slate-900 leading-none">{selectedLog.host.name}</p>
                                    </div>
                                    <div className="grid grid-cols-2 gap-6">
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Unit Number</p>
                                            <p className="text-sm font-bold text-slate-900">{selectedLog.host.unit || 'N/A'}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Address</p>
                                            <p className="text-sm font-bold text-slate-900 line-clamp-1">{selectedLog.host.address || 'Internal'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="rounded-[2rem] bg-slate-50 p-6 ring-1 ring-slate-200/50">
                                <div className="mb-5 flex items-center gap-3">
                                    ...
                                    <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">Visitor Contact</h4>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Phone Number</p>
                            ...
                            {selectedLog.vehicle && (
                                <div className="rounded-[2rem] bg-slate-50 p-6 ring-1 ring-slate-200/50">
                                    <div className="mb-5 flex items-center gap-3">
                                        ...
                                        <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">Vehicle Details</h4>
                                    </div>
                                    <div className="grid grid-cols-2 gap-6">
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Make & Model</p>
                                            <p className="text-sm font-bold text-slate-900">{selectedLog.vehicle.make} {selectedLog.vehicle.model}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Plate Number</p>
                                            <p className="text-sm font-black text-indigo-600 uppercase tracking-widest">{selectedLog.vehicle.plate}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="rounded-[2rem] border-2 border-dashed border-slate-200 p-6">
                                <div className="mb-5 flex items-center gap-3">
                                    ...
                                    <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">Verification</h4>
                                </div>
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight">Timestamp</p>
                                        <p className="text-sm font-bold text-slate-900">{selectedLog.verified_at}</p>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight">Validated By</p>
                                        <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-2">
                                        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight">Purpose</p>
                        ...
                        <button 
                            ...
                            className="w-full rounded-[1.5rem] bg-slate-900 py-5 text-base font-black text-white shadow-2xl active:scale-[0.98] transition-transform"
                        >
                            Close Details
                        </button>
                    </div>
