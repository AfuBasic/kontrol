...
@section('content')
    @if($isExistingUser ?? false)
        <div class="badge" style="background-color: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd;">Role Update</div>
        <h1>Property Owner role added to your account</h1>

        <p>Hello <span class="bold">{{ $userName }}</span>,</p>

        <p>You have been assigned the <span class="bold">Property Owner</span> role at <span class="bold">{{ $estateName }}</span>. This role has been added to your Kontrol account, and you can now manage properties and view estate updates directly from your dashboard.</p>
    @else
    ...
    <div class="button-container">
        @if($isExistingUser ?? false)
            <a href="{{ $invitationUrl }}" class="button shadow" style="background-color: #0284c7; box-shadow: 0 4px 14px 0 rgba(2, 132, 199, 0.3);">Go to Dashboard</a>
        @else
    ...
    @if($isExistingUser ?? false)
        <div style="background-color: #f0f9ff; border-radius: 12px; padding: 20px; font-size: 14px; color: #0369a1; border: 1px solid #bae6fd;">
            ...
            If you are currently signed in, simply switch to <strong>{{ $estateName }}</strong> from your account menu to access your property owner portal.
        </div>
    @else
        @if(!($isResend ?? false))
