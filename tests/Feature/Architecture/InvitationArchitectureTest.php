...
test('1. Bulk import creates ghost users and pending invitations', function () {
    ...
    // Assert ghost users WERE created in users table with null password
    expect(User::whereIn('email', $emails)->count())->toBe(2);

    // Assert pending invitations were created
    expect(Invitation::withoutZoneIsolation()->where('estate_id', $this->estateA->id)->whereIn('email', $emails)->count())->toBe(2);
});
...
test('9. Already-member user can receive a new invitation which updates their membership upon acceptance', function () {
...
test('10. Re-importing the same email does not create uncontrolled duplicate pending invitations', function () {
    ...
    expect(Invitation::withoutZoneIsolation()->where('estate_id', $this->estateA->id)->where('email', 'repeat@example.com')->count())->toBe(1);
});
...
test('19. Invalid duplicate emails are reported and 20. Valid rows remain successfully processed', function () {
    ...
    expect(Invitation::withoutZoneIsolation()->where('estate_id', $this->estateA->id)->count())->toBe(2);
});

test('20. Resident in estate can be invited to new roles and duplicate roles', function () {
    ...
    // 1. Inviting as resident again -> now creates invitation to refresh/resend
    $residentInv = app(CreateInvitationAction::class)->execute(
        email: 'multi_role@example.com',
        estate: $this->estateA,
        relationshipType: 'resident'
    );
    expect($residentInv)->not->toBeNull();
