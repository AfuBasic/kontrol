...
it('calculates fees exactly and charges 0.5% processing fee when resident has no active subscription', function () {
    ...
    expect(Payment::withoutGlobalScope(PaymentScope::class)->where('collection_assignment_id', $assignment->id)->value('amount'))->toBe(1000);
});
...
it('accepts partial payment amounts when remaining balance is at least 20% of the bill', function () {
    ...
    expect(Payment::withoutGlobalScope(PaymentScope::class)->where('collection_assignment_id', $assignment->id)->value('amount'))->toBe(25000);
});
...
it('still allows full payment when remaining balance is below 20% of the bill', function () {
    ...
    expect(Payment::withoutGlobalScope(PaymentScope::class)->where('collection_assignment_id', $assignment->id)->value('amount'))->toBe(15000);
});
