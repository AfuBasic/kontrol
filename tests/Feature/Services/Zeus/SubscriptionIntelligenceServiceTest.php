...
it('calculates plan analytics correctly', function () {
    ...
    ResidentSubscription::factory()->count(2)->create(['plan_id' => $plan1->id, 'status' => 'active']);
    ResidentSubscription::factory()->count(3)->create(['plan_id' => $plan2->id, 'status' => 'active']);
    ...
    expect($basicData['residents_count'])->toBe(2);
    ...
    expect($premiumData['residents_count'])->toBe(3);
...
it('calculates renewal cohorts correctly', function () {
    ...
    // 0-30 days
    ResidentSubscription::factory()->count(2)->create([
        'plan_id' => $plan->id,
        'status' => 'active',
    ...
    // 31-60 days
    ResidentSubscription::factory()->count(1)->create([
        'plan_id' => $plan->id,
        'status' => 'active',
        'current_period_end' => Carbon::now()->addDays(45),
    ]);

    // 61-90 days
    ResidentSubscription::factory()->count(3)->create([
        'plan_id' => $plan->id,
        'status' => 'active',
        'current_period_end' => Carbon::now()->addDays(75),
    ]);
...
it('calculates upgrade downgrade matrix correctly', function () {
    ...
    $subscription = ResidentSubscription::factory()->create(['plan_id' => $expensivePlan->id]);
