...
class PaymentScope implements Scope
{
    ...
    public function apply(Builder $builder, Model $model): void
    {
        ...
        if ($context->isZoneScoped()) {
            $builder->whereHas('assignment', function ($q) use ($context) {
                $q->whereHas('property', function ($pq) use ($context) {
                    $pq->where('zone_id', $context->zoneId);
                })->orWhere(function ($pq) use ($context) {
                    $pq->whereNull('property_id')
